...
def getMeshFromPoints(points):
    points = np.array(points)
    cloud = pv.PolyData(points)
    # cloud.rotate_y(-90, cloud.center)
    mesh = cloud.delaunay_2d()
    # mesh.rotate_y(90, mesh.center)
...
def generateSTL(IMAGE_FILES):
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for idx, file in enumerate(IMAGE_FILES):
            ...
            image = cv2.resize(image, (DESIRED_DIM, DESIRED_DIM))
    ...
    meshes = [getMeshFromPoints(points) for points in point_output]
    ...
    new_mesh.rotate_y(-90, new_mesh.center)
    new_mesh.delaunay_2d(inplace=True)
    new_mesh.rotate_y(90, new_mesh.center)
